...
export function ItemList({productList,loading,error}){
    ...
    return(
        ...
        {productList.length!==0 ? productList.map((product,index)=>
            <Link to={`/itemDetail/${product.apiId}`} key={product.apiId} style={{textDecoration:'none'}}> <ItemCard  title={product.title} description={product.description} price={product.price} pictureURL={GetFirstPicture(product.picture0)} stock={product.stock}></ItemCard></Link>): !loading && <LandingPage message={'No tenemos esos articulos, pero ya mismo'}></LandingPage>
        }
